...
class ILI9225:
    ...
    def window_begin(self, x, y, width, height):
        ...
        (startAddrHi, startAddrLo) = self.address(x, y) # type: ignore
        (endAddrHi, endAddrLo) = self.address(x + width -1, y + height - 1) # type: ignore
    ...
    def print(self, text, x, y, font, fg_color = COLOR_WHITE, bg_color = COLOR_BLACK, align = ALIGN_LEFT, iTxtBoxWidth = None):
        text_width = 0
        text_height = 0
        ...
        if iTxtBoxWidth is None:
            iTxtBoxWidth = text_width

        # If iTxtBoxWidth is chosen, and the box width is greater than the text with,
        # calculate how many pixels we need to pad before and after the text:
        if iTxtBoxWidth is None:
            iTxtBoxWidth = text_width

        if align == ALIGN_RIGHT:
            pad_left = iTxtBoxWidth - text_width
            pad_right = 0
        elif align == ALIGN_CENTER:
            pad_left = (iTxtBoxWidth - text_width) // 2
            pad_right = iTxtBoxWidth - pad_left - text_width
        else: # ALIGN_LEFT
            pad_right = iTxtBoxWidth - text_width
            pad_left = 0

        # First draw the padding to the left of the text:
        if pad_left > 0:
        ...
        # And finally draw the padding to the right:
        if pad_right > 0:
            self.fill_rect(x, y, pad_right, char_height, bg_color)
            x = iTxtBoxWidth

        return x


    def print_old(self, text, x, y, font, fg_color = COLOR_WHITE, bg_color = COLOR_BLACK, align = ALIGN_LEFT, x2 = None):
        ...
        # Bug fix (KBKJ): Miscalculation of padding in the original code
        #pad_left = 0 if align == ALIGN_LEFT else (x2 - x - text_width) if align == ALIGN_RIGHT else (x2 - x - text_width) // 2
        pad_left = 0 if align == ALIGN_LEFT else (x2 - x - text_width) if align == ALIGN_RIGHT else ((x2 - text_width) // 2)
        ...
        if x2 > x:
            self.fill_rect(x, y, x2 - x + x_original, char_height, bg_color)
            x = x2
